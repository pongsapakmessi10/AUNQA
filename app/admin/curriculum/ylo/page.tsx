...
export default function YLOPage() {
  const [items, setItems] = useState([
    {
      id: 1,
      detail: "สามารถประยุกต์ใช้ความรู้ด้านวิทยาการคอมพิวเตอร์ในการแก้ปัญหาได้",
    },
    {
      id: 2,
      detail: "มีจริยธรรมและจรรยาบรรณในวิชาชีพ",
    },
  ]);
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="py-4 px-6 font-bold text-[#1b3860] text-sm w-full">ข้อมูล YLO</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center p-5 border-b border-gray-100">
              <h2 className="text-xl font-bold text-[#1b3860]">เพิ่มข้อมูล YLO</h2>
              <button 
                onClick={closeModal}
            ...
            <form onSubmit={handleAddItem} className="p-5 space-y-4">
              <div>
                <label htmlFor="detail" className="block text-sm font-bold text-gray-700 mb-1.5">
                  รายละเอียดข้อมูล YLO
                </label>
                <textarea
                  id="detail"
                  value={newDetail}
                  onChange={(e) => setNewDetail(e.target.value)}
                  placeholder="กรอกข้อมูล YLO"
                  rows={4}
