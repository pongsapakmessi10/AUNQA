...
import { LayoutDashboard, BookPlus, PenSquare, FileSpreadsheet, LogOut, ChevronRight, Menu, X } from "lucide-react";
...
export default function StudentSidebar() {
  ...
  const menuItems = [
    ...
    { name: "ใบกระจายโครงสร้างหลักสูตร", icon: FileSpreadsheet, href: "/student/curriculum" },
    { name: "เพิ่มรายวิชา", icon: BookPlus, href: "/student/add-course" },
    { name: "กรอกเกรด", icon: PenSquare, href: "/student/fill-grades" },
  ];
    ...
    <>
      ...
      <div className="md:hidden flex items-center justify-between bg-[#1b3860] text-white p-4 sticky top-0 z-40 shadow-md">
        <div className="flex items-center space-x-3">
          ...
          />
          <h2 className="font-bold uppercase tracking-wide text-sm">Student Portal</h2>
        </div>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-[#1b3860] text-white shadow-2xl flex flex-col font-sans border-r border-[#142946] z-50 transform transition-transform duration-300 ease-in-out md:sticky md:top-0 md:h-screen md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-8 flex flex-col items-center border-b border-white/10 bg-[#172f52]/50">
          ...
          />
          <h2 className="text-xl font-bold tracking-wide uppercase text-white">Student Portal</h2>
        ...
        <div className="p-6 border-t border-white/10 bg-[#172f52]">
          <div className="flex items-center space-x-3 mb-5">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-bold text-white truncate">สมชาย ใจดี</p>
              <p className="text-xs text-[#8b9cbd] font-medium mt-0.5 truncate">รหัสนักศึกษา: 6500000000</p>
            </div>
          </div>
